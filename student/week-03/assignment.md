...
Defend schema and index choices with measurements.
...
- [PostgreSQL tutorial](https://www.postgresql.org/docs/current/tutorial.html) — suggested 60 min
- [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/using-explain.html) — suggested 35 min

While reading, answer: what problem does this solve, which assumption can fail, and which observable signal would reveal failure? Topics this week: relational modeling, indexes, transactions, query plans.
...
```powershell
.\training-platform\scripts\start-week.ps1 -Week 3
.\training-platform\scripts\smoke-test.ps1 -Week 3
```
...
1. **Create constraints that reject invalid resources** — 40 minutes.
2. **Compare query plans before and after an index** — 40 minutes.
3. **Restart the database and prove committed data survives** — 40 minutes.

For the visible case for this week, run:

```powershell
.\training-platform\student\run-case.ps1 -Case week-03-practice
```
...
**Design and query a resource catalog.** Use the simulator at `http://localhost:8080`. Write acceptance criteria first, implement the smallest useful vertical slice, and preserve request IDs and other evidence. Use portable path APIs such as `pathlib.Path`; copying the supplied PowerShell commands is sufficient.
...
```powershell
.\training-platform\student\new-case.ps1 -Name my-week-03-case -Mode delay -DelayMs 700
.\training-platform\student\run-case.ps1 -Case my-week-03-case
```
