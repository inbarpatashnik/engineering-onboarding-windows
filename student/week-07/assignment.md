...
Explain what each test catches and misses.
...
- [pytest getting started](https://docs.pytest.org/en/stable/getting-started.html) — suggested 40 min
- [Python unittest](https://docs.python.org/3/library/unittest.html) — suggested 35 min

While reading, answer: what problem does this solve, which assumption can fail, and which observable signal would reveal failure? Topics this week: unit, integration, contract, property, and end-to-end testing.
...
```powershell
.\training-platform\scripts\start-week.ps1 -Week 7
.\training-platform\scripts\smoke-test.ps1 -Week 7
```
...
1. **Turn one production-like failure into a regression test** — 40 minutes.
2. **Separate unit, integration, and contract checks** — 40 minutes.
3. **Make a nondeterministic test deterministic** — 40 minutes.

For the visible case for this week, run:

```powershell
.\training-platform\student\run-case.ps1 -Case week-07-practice
```
...
**Create a layered automated test suite.** Use the simulator at `http://localhost:8080`. Write acceptance criteria first, implement the smallest useful vertical slice, and preserve request IDs and other evidence. Use portable path APIs such as `pathlib.Path`; copying the supplied PowerShell commands is sufficient.
...
```powershell
.\training-platform\student\new-case.ps1 -Name my-week-07-case -Mode delay -DelayMs 700
.\training-platform\student\run-case.ps1 -Case my-week-07-case
```
