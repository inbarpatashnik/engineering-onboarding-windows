...
Plan detection, rollback, and data compatibility.
...
- [Docker Compose how-to guides](https://docs.docker.com/compose/how-tos/) — suggested 45 min
- [GitHub Actions documentation](https://docs.github.com/actions) — suggested 45 min

While reading, answer: what problem does this solve, which assumption can fail, and which observable signal would reveal failure? Topics this week: CI/CD, containers, configuration, migrations, rollback.
...
```powershell
.\training-platform\scripts\start-week.ps1 -Week 12
.\training-platform\scripts\smoke-test.ps1 -Week 12
```
...
1. **Build and run an immutable container image** — 40 minutes.
2. **Create failed-health-check and incompatible-config cases** — 40 minutes.
3. **Rehearse a rollback without deleting data** — 40 minutes.

For the visible case for this week, run:

```powershell
.\training-platform\student\run-case.ps1 -Case week-12-practice
```
...
**Create a safe release runbook.** Use the simulator at `http://localhost:8080`. Write acceptance criteria first, implement the smallest useful vertical slice, and preserve request IDs and other evidence. Use portable path APIs such as `pathlib.Path`; copying the supplied PowerShell commands is sufficient.
...
```powershell
.\training-platform\student\new-case.ps1 -Name my-week-12-case -Mode delay -DelayMs 700
.\training-platform\student\run-case.ps1 -Case my-week-12-case
```
