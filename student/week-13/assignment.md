...
Lead a clear evidence-based incident review.
...
- [Google SRE: managing incidents](https://sre.google/sre-book/managing-incidents/) — suggested 55 min
- [Google SRE: postmortem culture](https://sre.google/sre-book/postmortem-culture/) — suggested 35 min

While reading, answer: what problem does this solve, which assumption can fail, and which observable signal would reveal failure? Topics this week: system synthesis, incident response, communication, remediation.
...
```powershell
.\training-platform\scripts\start-week.ps1 -Week 13
.\training-platform\scripts\smoke-test.ps1 -Week 13
```
...
1. **Start from an unknown compound failure** — 40 minutes.
2. **Maintain a timestamped hypothesis log** — 40 minutes.
3. **Stabilize first, then write a blameless post-incident review** — 40 minutes.

For the visible case for this week, run:

```powershell
.\training-platform\student\run-case.ps1 -Case week-13-practice
```
...
**Diagnose and stabilize a multi-symptom incident.** Use the simulator at `http://localhost:8080`. Write acceptance criteria first, implement the smallest useful vertical slice, and preserve request IDs and other evidence. Use portable path APIs such as `pathlib.Path`; copying the supplied PowerShell commands is sufficient.
...
```powershell
.\training-platform\student\new-case.ps1 -Name my-week-13-case -Mode delay -DelayMs 700
.\training-platform\student\run-case.ps1 -Case my-week-13-case
```
