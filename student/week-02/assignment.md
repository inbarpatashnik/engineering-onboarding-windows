...
Follow relevant linked subsections and take working notes. Answer: what problem does this solve, which assumption can fail, and which observable signal would reveal failure? Topics this week: HTTP semantics, REST, validation, error models, retries.
...
1. **Classify ten responses as success, client, server, or transient failure** — suggested 1.5 hours.
2. **Measure a timeout and preserve the request ID** — suggested 1.5 hours.
3. **Create a malformed-response case and make the client fail clearly** — suggested 1.5 hours.
