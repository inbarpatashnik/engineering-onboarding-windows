...
While reading, answer: what problem does this solve, which assumption can fail, and which observable signal would reveal failure? Topics this week: document stores, graph traversal, denormalization tradeoffs.
...
1. **Model the same ownership data as documents and as a graph** — 40 minutes.
2. **Write three access patterns before choosing a model** — 40 minutes.
3. **Create a cyclic dependency case and bound traversal** — 40 minutes.

For the visible case for this week, run:

```powershell
.\training-platform\student\run-case.ps1 -Case week-04-practice
```
...
```powershell
.\training-platform\student\new-case.ps1 -Name my-week-04-case -Mode delay -DelayMs 700
