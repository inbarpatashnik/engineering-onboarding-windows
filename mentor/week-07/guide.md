# Mentor guide — Week 7: Testing Strategy

> Restricted: contains review guidance and hidden assessment details.

## Review intent

The student should demonstrate: explain what each test catches and misses. Focus the review on test boundaries, deterministic fixtures, contract ownership. Do not reward extra architecture that lacks evidence, and do not operate or repair the normal environment for the student.
...
```powershell
+.\training-platform\mentor\set-fault.ps1 -Preset week-07
+```

Expected injected behavior is recorded in `training-platform\mentor\incident-presets\week-07.json`. Ask the student to diagnose before revealing it. Finish with `.\training-platform\mentor\reset-faults.ps1`.
