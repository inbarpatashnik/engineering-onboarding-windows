# Mentor guide — Week 1: Engineering Foundations and Windows Tooling

> Restricted: contains review guidance and intervention details.

## Review intent

The student should demonstrate: explain a failure from evidence instead of guessing. Focus the review on PowerShell quoting, venv activation, path portability, small commits. Do not reward extra architecture that lacks evidence.
...
```powershell
+.\training-platform\mentor\set-fault.ps1 -Preset week-01
+```

Expected injected behavior is recorded in `training-platform\mentor\incident-presets\week-01.json`. Ask the student to diagnose before revealing it. Finish with `.\training-platform\mentor\reset-faults.ps1`.
