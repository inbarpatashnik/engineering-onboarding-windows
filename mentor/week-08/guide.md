...
The student should demonstrate: diagnose latency using correlated telemetry. Focus the review on cardinality, correlation IDs, RED/USE signals, sampling. Do not reward extra architecture that lacks evidence, and do not operate or repair the normal environment for the student.
...
```powershell
+.\training-platform\mentor\set-fault.ps1 -Preset week-08
+```

Expected injected behavior is recorded in `training-platform\mentor\incident-presets\week-08.json`. Ask the student to diagnose before revealing it. Finish with `.\training-platform\mentor\reset-faults.ps1`.
