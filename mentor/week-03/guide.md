# Mentor guide — Week 3: Data Modeling and SQL

> Restricted: contains review guidance and hidden assessment details.

## Review intent

The student should demonstrate: defend schema and index choices with measurements. Focus the review on normalization, constraints, transaction boundaries, measured indexes. Do not reward extra architecture that lacks evidence, and do not operate or repair the normal environment for the student.
...
```powershell
+.\training-platform\mentor\set-fault.ps1 -Preset week-03
+```

Expected injected behavior is recorded in `training-platform\mentor\incident-presets\week-03.json`. Ask the student to diagnose before revealing it. Finish with `.\training-platform\mentor\reset-faults.ps1`.
